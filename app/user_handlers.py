...
async def cb_show_stars_menu(callback: CallbackQuery, state: FSMContext):
    ...
    buttons = [
        [InlineKeyboardButton(text="🔥 Купить в 15 раз дешевле через DonationAlerts", callback_data="btn_buy_callback")]
    ]
    ...
    text = (
        ...
        "цена при оплате через Stars <b>в 15 раз выше</b>, чем через DonationAlerts.\n\n"
        "💡 <i>Рекомендуем оплачивать через DonationAlerts — это в 15 раз дешевле, без комиссий и зачисляется моментально с любой карты или СБП!</i>\n\n"
