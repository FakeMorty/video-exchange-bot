    ...
    <div class="modal-content">
      ...
      <div style="display: flex; flex-direction: column; gap: 8px; max-height: 250px; overflow-y: auto;">
        <div class="package-item">
          <div><b>⚡ 500 монет</b><br><small style="color: var(--tg-theme-hint-color);">Быстрый старт · 750 ★</small></div>
          <button class="banner-btn" style="padding: 6px 12px; font-size: 11px;" onclick="buyCoins('pack_50')">Купить</button>
        </div>
        <div class="package-item">
          <div><b>🔥 1 000 монет</b><br><small style="color: var(--tg-theme-hint-color);">Популярный пакет · 1 500 ★</small></div>
          <button class="banner-btn" style="padding: 6px 12px; font-size: 11px;" onclick="buyCoins('pack_100')">Купить</button>
        </div>
        <div class="package-item">
          <div><b>💎 2 200 монет</b><br><small style="color: var(--tg-theme-hint-color);">Самый выгодный · 3 000 ★</small></div>
...
async def auto_broadcast_worker(bot):
    """Периодическая авто-рассылка: раз в 20мин–6ч (случайно) рассылает всем
    активным пользователям одно сообщение из пула ротации.

    Пул живёт в БД (таблица promo_messages: дефолтные шаблоны + добавленные
    админом — правятся в «📣 Промо-рассылки» → «📋 Авто-рассылки (ротация)»),
    плюс на каждое активное событие добавляется его карточка (с фото, если
    есть) — события напоминают о себе по тому же принципу, что и промо.
    """
    import asyncio
    import random
    from app.db import async_session
    from app.models import User
    from app.services import seed_default_promo_messages, get_auto_broadcast_pool
    from sqlalchemy import select

    await asyncio.sleep(60)  # подождать старта бота

    # Одноразовый сид дефолтных шаблонов (по маркеру promo_messages_seeded_v1;
    # удалённые админом шаблоны не воскресают).
    try:
        async with async_session() as session:
            seeded = await seed_default_promo_messages(session)
            if seeded:
                logger.info(f"Promo rotation: seeded {seeded} default messages.")
    except Exception:
        logger.exception("Promo rotation seed failed (non-critical)")
    ...
    while True:
        try:
            ...
            if not pool:
                logger.info("Promo rotation: пул пуст (нет шаблонов и событий) — пропуск цикла.")
                continue

            # Выбираем случайное сообщение из актуального пула
            ...
            logger.exception("auto_broadcast_worker cycle failed")
