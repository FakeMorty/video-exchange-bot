...
from app.services import (
    ...
    get_recent_feedback, get_active_sale, get_active_events)
...
async def trusted_add_process(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        existing = (await session.execute(
            select(TrustedUploader, Event, ActiveSale, OfferParticipation).where(
...
async def _check_admin_internal(tg_id: int) -> bool:
...
async def event_create_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🎉 Шаг 1/7: Введите название события:")
    await callback.answer()
...
async def event_name(message: Message, state: FSMContext):
    ...
    await message.answer("📝 Шаг 2/7: Введите текст события:")
...
async def event_description(message: Message, state: FSMContext):
    ...
    await message.answer("💰 Шаг 3/7: Введите размер скидки (1-99):")
...
async def event_discount(message: Message, state: FSMContext):
    ...
    await message.answer("📅 Шаг 4/7: Сколько дней будет длиться?")
...
async def event_duration(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Шаг 5/7 — На что скидка?", reply_markup=event_applies_keyboard(applies))
...
async def event_applies_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🖼 Шаг 6/7 — Фото или напишите 'пропустить':")
    await callback.answer()
...
async def event_image(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Создать", callback_data="event_confirm_yes"), 
         InlineKeyboardButton(text="❌ Отмена", callback_data="admin_events_menu")]
    ])
    await message.answer(f"🎉 Создать событие {escape(data['name'])}?", parse_mode="HTML", reply_markup=kb)
...
async def event_confirm_yes(callback: CallbackQuery, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        await session.commit()
    await state.clear()
    await callback.message.answer("✅ Событие успешно создано!")
    await callback.answer()
