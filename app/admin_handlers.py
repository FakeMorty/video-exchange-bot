...
async def db_open(callback: CallbackQuery):
    ...
    table_name = parts[1]
    try:
        offset = max(0, int(parts[2])) if len(parts) > 2 else 0
    except Exception:
        offset = 0
...
async def mod_approve(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if video:
            ...
            if uploader:
                try:
                    await callback.bot.send_message(uploader.telegram_id, f"✅ Ваше видео #{video_id} одобрено!")
                except:
                    pass
...
async def reject_reason(callback: CallbackQuery):
    ...
    reason_map = {"duplicate": "Дубликат", "off_topic": "Не по теме", "forbidden": "Запрещёнка", "other": "Другая"}
    ...
    async with async_session() as session:
        ...
        if video:
            ...
            if uploader:
                try:
                    await callback.bot.send_message(uploader.telegram_id, f"❌ Видео #{video_id} отклонено: {reason_text}")
                except:
                    pass
...
async def event_create_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🎉 Шаг 1/7: Название:")
    await callback.answer()
...
async def event_name(message: Message, state: FSMContext):
    ...
    await message.answer("📝 Шаг 2/7: Текст события:")
...
async def event_description(message: Message, state: FSMContext):
    ...
    await message.answer("💰 Шаг 3/7: Скидка (1-99%):")
...
async def event_discount(message: Message, state: FSMContext):
    ...
    await message.answer("📅 Шаг 4/7: Длительность (дней):")
...
async def event_duration(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    await state.update_data(duration_days=days, applies={"vip": False, "coins": False, "lootbox": False, "cases": False})
    await state.set_state(EventCreationState.waiting_applies)
    await message.answer("✅ Шаг 5/7: На что?", reply_markup=event_applies_keyboard({"vip": False, "coins": False, "lootbox": False, "cases": False}))
...
async def event_applies_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🖼 Шаг 6/7: Фото или 'пропустить':")
    await callback.answer()
...
async def event_image(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Создать", callback_data="event_confirm_yes"), InlineKeyboardButton(text="❌ Отмена", callback_data="admin_events_menu")]
    ])
    await message.answer(f"🎉 <b>Подтверждение</b>\nНазвание: {escape(data['name'])}\nСкидка: {data['discount_percent']}%", parse_mode="HTML", reply_markup=kb)
...
async def admin_sale_duration(message: Message, state: FSMContext):
    ...
    await message.answer("Текст объявления (HTML):")
...
async def admin_sale_finish(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        await session.commit()
    await state.clear()
    await message.answer("✅ Глобальная акция запущена!")
...
async def admin_broadcast_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📢 Текст рассылки (HTML):")
    await callback.answer()
