...
# BUG FIX #1: check_admin была async, но вызывалась без await в cmd_admin_redirect
# в user_handlers.py. Оставляем async — важно использовать await везде при вызове.
...
async def admin_back(callback: CallbackQuery):
    # BUG FIX #2: cmd_admin принимает Message, а не CallbackQuery.
    # Нужно передавать callback.message, а не сам callback.
...
async def add_admin_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Введите Telegram ID пользователя для назначения в админы:"
    )
    await callback.answer()
...
async def process_add_admin(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Пользователь {telegram_id} назначен админом.")
    await state.clear()
