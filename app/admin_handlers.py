...
async def cmd_admin(message: Message):
    ...
    if not video:
        await message.answer(f"\u2705 No videos for moderation. (Pending: {pending_count})")
        return
    ...
    await message.answer_video(
        ...
        caption=(
            f"<b>Moderation</b>\n"
            f"Video ID: {video.id}\n"
            f"Author (user_id): {video.uploader_user_id}\n"
            f"Duration: {video.duration_seconds or '?'} sec\n"
            f"In queue: {pending_count}"
        ),
        parse_mode="HTML",
...
async def cb_approve(callback: CallbackQuery):
    ...
    if video:
        await callback.message.edit_caption(
            caption=f"\u2705 Video #{video_id} approved! Author got +0.5 coins.",
        )
    else:
        await callback.message.edit_caption(
            caption=f"Video #{video_id} not found.",
        )
...
async def cb_reject(callback: CallbackQuery):
    ...
    await callback.answer("Choose rejection reason")
...
async def cb_reject_reason(callback: CallbackQuery):
    ...
    if video:
        await callback.message.edit_caption(
            caption=f"\u274c Video #{video_id} rejected.\nReason: {reason}",
        )
    else:
        await callback.message.edit_caption(
            caption=f"Video #{video_id} not found.",
...
async def _send_next_pending(message: Message, admin_id: int):
    ...
    if not video:
        await message.answer("\u2705 Queue empty! All videos processed.")
        return
