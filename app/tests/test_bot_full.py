...
async def test_stars_discount_affects_displayed_vip_and_pack_prices():
    ...
    async with Session() as session:
        ...
        assert packs["pack_50"]["stars"] == 563
        assert packs["pack_100"]["stars"] == 1125
        assert packs["pack_200"]["stars"] == 2250
...
async def test_suite_growth_pack():
    ...
    async with async_session() as s:
        ...
        assert sp and sp["stars"] == 135 and sp["coins"] == 500, f"starter price: {sp}"
        pay = await create_payment(s, uid2, "starterpack")
        assert pay.stars_amount == 135
    ...
    print("PASS  growth B: starter pack 135 Stars/500, одноразовость")
